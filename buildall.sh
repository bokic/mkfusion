...
sed 's/__ARCH__/'$(dpkg --print-architecture)'/g;s/__VERSION__/0.5.0.'$(svn info --xml | xmllint --xpath 'string(/info/entry/@revision)' -)'/g' install/ubuntu/mkfusion_control > install/ubuntu/mkfusion/DEBIAN/control
...
fakeroot dpkg -b install/ubuntu/mkfusion mkfusion_0.5.0.$(svn info --xml | xmllint --xpath 'string(/info/entry/@revision)' -)_$(dpkg --print-architecture).deb
...
sed 's/__ARCH__/'$(dpkg --print-architecture)'/g;s/__VERSION__/0.5.0.'$(svn info --xml | xmllint --xpath "string(/info/entry/@revision)" -)'/g' install/ubuntu/cfeditor_control > install/ubuntu/cfeditor/DEBIAN/control
...
fakeroot dpkg -b install/ubuntu/cfeditor cfeditor_0.5.0.$(svn info --xml | xmllint --xpath 'string(/info/entry/@revision)' -)_$(dpkg --print-architecture).deb
